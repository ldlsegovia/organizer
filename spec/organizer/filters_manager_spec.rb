  ...
  describe "#apply" do
    context "with default filters" do
      ...
      it "returns filtered collection" do
        ...
        expect(result.first.attr1).to eq(collection.second.attr1)
      end
    ...
    context "with normal filters" do
      ...
      it "returns filtered collection" do
        expect(subject.apply(collection).size).to eq(3)
        expect(subject.apply(collection, enabled_filters: [:filter1]).size).to eq(2)
        expect(subject.apply(collection, enabled_filters: [:filter1, :filter2]).size).to eq(1)
      end
    end
    ...
    context "with filters with value" do
      ...
      it "returns filtered collection" do
        expect(subject.apply(collection).size).to eq(3)
        expect(subject.apply(collection, filters: { filter1: 4 }).size).to eq(2)
        expect(subject.apply(collection, filters: { filter1: 4, filter2: 80 }).size).to eq(1)
      end
    end

    context "with autogenerated filters" do
      it "returns filtered collection" do
        expect(subject.apply(collection, filters: { attr1_eq: 4 }).size).to eq(1)
        expect(subject.apply(collection, filters: { attr2_contains: "ia" }).size).to eq(1)
      end
