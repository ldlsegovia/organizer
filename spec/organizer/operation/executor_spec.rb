...
describe Organizer::Operation::Executor do
  ...
  describe "#execute_on_source" do
    ...
    it "returns collection items with new attribute" do
      expect(subject.execute_on_source(@operations, collection).first.result_attr).to eq(44)
    end
    ...
    context "with nested operations" do
      ...
      it "returns collection items with new attribute" do
        result = subject.execute_on_source(@operations, collection).first
      ...
      context "with invalid item attribute" do
        ...
        it "raise exception" do
          expect { subject.execute_on_source(@operations, collection) }.to(
