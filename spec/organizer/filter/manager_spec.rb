  ...
  describe "#apply" do
    ...
    context "with autogenerated filters" do
      it "returns filtered collection" do
        ...
        expect(subject.apply(collection, filters: { first_name_contains: "Manuel" }).size).to eq(1)
      end
    ...
    it "has generated filters" do
      item_hash_keys.each do |attribute|
        [:eq, :not_eq, :gt, :lt, :goet, :loet, :starts, :ends, :contains].each do |sufix|
          ...
          expect(subject.send(:filters_with_values).find_by_name(filter_name).item_name).to eq(filter_name)
        end
      end
    end
